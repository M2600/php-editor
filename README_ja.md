...
このセクションは php-editor の基本的な使い方を説明します。内部の仕組みには触れません。

### 画面構成
- 左: エクスプローラー（フォルダ/ファイル一覧）
- 中央: エディタ（コード編集）／下部: コンソール（メッセージ表示）
- 右: AIチャット（アシスタント）
- 上部: タイトル・メニュー（テーマ切替など）

ヒント: 上部メニューの「Theme」ボタンでライト/ダークを切り替えできます。

### エクスプローラーの使い方
- フォルダの三角アイコン（▶/▼）で展開・折りたたみ
- 「../」で1つ上の階層に移動
- ファイルをクリックすると編集画面に表示されます
- 未保存の変更があるファイルは「*」が表示されます

上部メニュー（エクスプローラー）
- New File: 新規ファイルの作成
- ⋮（その他）: New Folder（新規フォルダ）/ Upload（ファイルアップロード）

各フォルダのメニュー（⋮）
- rename / new file / new folder / upload / delete

各ファイルのメニュー（⋮）
- rename / move / duplicate / delete

### エディタ（コード編集）
- PHP/HTML/CSS/JS などをシンタックスハイライト付きで編集できます
- ファイルを切り替えても、カーソル位置やUndo/Redoの履歴が保持されます
- 画像などテキスト以外はビューアでプレビュー表示されます

主な操作
- 保存: 上部の [Save] ボタン、または Ctrl+S（macOSは ⌘S）
- 実行: 上部の [Run] ボタン、または F10（新しいタブで実行）
- QRコード: [QR Code] でURLのQRを表示（スマホでアクセス）
- コンソール: 保存・読み込み・実行などのメッセージを表示（自動スクロール）

### AIチャットの使い方
- 右側の「Chat」パネルからAIに質問や依頼ができます
- モデル選択: 上部のプルダウンからAIモデルを選択
- 履歴クリア: 「クリア」で現在の会話を消去
- 送信: Enter（改行は Shift+Enter）

コードブロック（AIの返答内）
- 「コピー」: コードをクリップボードにコピー
- 「コードに適用」: エディタで開いているファイルに提案コードを反映
   - 状態表示: 適用中（🔄）→ 成功（✅）→ 自動で戻る／エラー（❌）

ファイルコンテキスト
- 開いているファイル名がチャット上部に表示され、AIへの質問に文脈を付与します
- ファイルを切り替えるとコンテキストも更新されます

### よく使うショートカット
- 保存: Ctrl+S（macOSは ⌘S）
- 実行: F10
- チャット送信: Enter（改行は Shift+Enter）
- Undo/Redo: 一般的な編集ショートカット（例: Ctrl+Z / Ctrl+Y）

### アップロード手順
1) エクスプローラー上部の「⋮」→ Upload を選択
2) ファイル選択ダイアログでアップロード
3) 完了後、自動で一覧が更新されます

### 困ったときは（ユーザー向け）
- 実行が開かない: ブラウザのポップアップブロックを解除
- 保存できない: 権限・ログイン状態を確認
- AIが応答しない: モデル選択とネットワークを確認
- 表示が崩れた: ページを再読み込み or テーマ切替後にリトライ
