...
async function main(){
    ...
    const changeJsonMode = (isJson) => {
        if (isJson) {
            ...
            try {
                    ...
                    // 既存のアイテムをクリア
                    dictMenu.items = [];
                    dictMenu.itemElements.forEach(el => el.remove());
    ...
    // シンプルモード: DictMenu
    const dictMenu = editor.createDictMenu(null, {});
    dictMenu.addButton();
    ...
    if (savedPostCheckState !== null) {
        try {
            ...
            if (isJsonMode) {
                ...
                dictMenu.setEnabled(postCheckEnabled);
            }
